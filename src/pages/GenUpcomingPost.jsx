  ...
  const handleGenerate = async (matchId) => {
    ...
    try {
      ...
      const res = await axios.get(
        `https://matchgen-backend-production.up.railway.app/api/graphicpack/match/${matchId}/generate-upcoming/`,
        {
          headers: {
      ...
      setSnackbar({ open: true, message: "Upcoming post generated!" });
  
      setMatches(prev => {
        if (Array.isArray(prev)) {
          return prev.map(m => m.id === matchId ? { ...m, upcoming_post_url: res.data.url } : m);
        }
        return prev;
      });
    } catch (err) {
      console.error("Error generating upcoming post", err);
      setSnackbar({ open: true, message: "Failed to generate upcoming post." });
    } finally {
      setLoadingId(null);
    }
  };
          ...
          >
            ...
            <Container sx={{ py: 4, width: '100%' }}>
              ...
              <Typography variant="body1" color="text.secondary" sx={{ mb: 3 }}>
                Generate social media posts for upcoming matches
              </Typography>
                    ...
                    {matches.length === 0 ? (
                      ...
                      matches.map((match) => (
                        <TableRow key={match.id}>
                          ...
                          <TableCell>
                            {match.upcoming_post_url ? (
                              <a href={match.upcoming_post_url} target="_blank" rel="noreferrer">View</a>
                            ) : (
                              "—"
                            )}
                          </TableCell>
...
export default GenUpcomingPost;
